...
class RaydiumNewPools:
    ...
    async def subscribe_to_log(self):
        async with connect(self.BASE_WSS_ENDPOINT) as websocket:
            await websocket.logs_subscribe(RpcTransactionLogsFilterMentions(self.RAYDIUM_AUTHORITY),commitment="finalized")
            ...
            while True:
                ...
                if hasattr(_result,'value'):
                    ...
                    if any("Program log: Error: The amm account owner is not match with this program" in log for log in logs):
                        print(log_signature)
                else:
